...
public class ReservationDAO {
    ...
    public static Double caReservationM(String idLieu, int mois){
        try {
            PreparedStatement stmt = DBConnex.connexxion("SELECT tarifDemiJournee FROM salle, reservation WHERE idLieu=? AND MONTH(dateResa) =? AND salle.idSalle=reservation.idSalle ");
            stmt.setString(1,idLieu);
            stmt.setInt(2,mois);
            ResultSet rs = stmt.executeQuery();
            Double caResa = 0.0;
    ...
    public static Double caReservationA(String idLieu, int annee){
        try {
            PreparedStatement stmt = DBConnex.connexxion("SELECT tarifDemiJournee FROM salle, reservation WHERE idLieu=? AND YEAR(dateResa) =? AND salle.idSalle=reservation.idSalle ");
            stmt.setString(1,idLieu);
            stmt.setInt(2,annee);
            ResultSet rs = stmt.executeQuery();
            Double caResa = 0.0;
